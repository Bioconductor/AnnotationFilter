...
flt <- convertFilterExpression(symbol == "BCL2" & tx_biotype == "protein_coding")
flt
```
...
setMethod("queryForEnsDb", "CharacterFilter", function(object, ...) {
    paste(object@field, .conditionForEnsDb(object),
...
`@field` slot of the `AnnotationFilter`. This is not the case for all columns in
an `EnsDb` database, thus, an additional function would be required here, that
