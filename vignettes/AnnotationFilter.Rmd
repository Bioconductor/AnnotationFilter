...
vignette: >
    ...
    %\VignetteDepends{EnsDb.Hsapiens.v75,BiocStyle,RSQLite}
---
...
## Define how the second filter pair in ( brackets should be combined.
...
dbcon <- dbconn(EnsDb.Hsapiens.v75)

## What tables do we have?
dbListTables(dbcon)

## What fields are there in the gene table?
dbListFields(dbcon, "gene")

```

Many, but not all, of the columns/fields corresponds to the default `field`
value of `AnnotationFilter` classes, such as *gene_id* or *seq_name*. For those
without a match we would have to re-map their field value to the respective
column names in the database (e.g. we would want to access the *gene_seq_start*
column for the `GeneStartFilter`). There are many possibilities to do this, one
would be to implement a function to extract the field value specific for the
database e.g. `ensDbField` that returns the default `field` value for
`AnnotationFilter` objects which field matches the database column name and the
actual name of the column in the database otherwise.

We could now implement a simple `doExtractGene` function that retrieves data
from the *gene* table and re-uses the `doFilter` function to extract specific
data. The parameter `x` is now the database connection object.

```{r doExtractSQL}

doExtractGene <- function(x, filter) {
    gene <- dbGetQuery(x, "select * from gene;")
    doExtract(gene, filter)
}

## Extract all genes on chromosome Y
chrY <- doExtractGene(dbcon, SeqNameFilter("Y"))
...
doExtractGene2 <- function(x, filter) {
    if (is(filter, "formula"))
        filter <- AnnotationFilter(filter)
    query <- paste0("select * from gene where ", where(filter))
    dbGetQuery(x, query)
}

chrY <- doExtractGene2(dbcon, ~ seq_name == "Y")
head(chrY)
...
```{r performance}
system.time(doExtractGene(dbcon, ~ seq_name == "Y"))

system.time(doExtractGene2(dbcon, ~ seq_name == "Y"))
